...
class BackgroundSyncUtility: BackgroundUtilityProtocol {
    ...
    func scheduleTaskOnAppBackground() {
        if let syncManager = profile.syncManager, syncManager.isSyncing {
            ...
            let request = BGProcessingTaskRequest(identifier: "mdutka.ios.sync.part1")
    ...
    private func setUpBackgroundSync() {
        BGTaskScheduler.shared.register(forTaskWithIdentifier: "mdutka.ios.sync.part1",
            ...
            self.profile.syncManager?.syncNamedCollections(why: .backgrounded, names: collection).uponQueue(.main) { _ in
                ...
                let request = BGProcessingTaskRequest(identifier: "mdutka.ios.sync.part2")
                request.earliestBeginDate = Date(timeIntervalSinceNow: 1)
                request.requiresNetworkConnectivity = true
                do {
                    try BGTaskScheduler.shared.submit(request)
        ...
        BGTaskScheduler.shared.register(forTaskWithIdentifier: "mdutka.ios.sync.part2",
                                        using: DispatchQueue.global()) { task in
