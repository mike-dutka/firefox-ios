...
class MultiWindowTests: IpadOnlyTestCase {
    let dotMenu = springboard.buttons["top-affordance:mdutka.ios.Fennec"]
    let splitView = springboard.buttons["top-affordance-split-view-button"]
    let dotMenuIdentifier = springboard.buttons.matching(identifier: "top-affordance:mdutka.ios.Fennec")
    ...
    private func openWindowFromTabSwitcher(windowsNumber: Int) {
        for  _ in 1...windowsNumber {
            dotMenu.waitAndTap()
            let cardOrgMozillaIosFennecButton = springboard.buttons["card:mdutka.ios.Fennec:"]
